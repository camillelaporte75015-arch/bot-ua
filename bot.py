...
# Garder en mémoire les GIFs déjà envoyés
used_gifs = []
...
async def ua(ctx):
    ...
    if len(used_gifs) >= len(GIF_LIST):
        used_gifs = []
        await ctx.send("🔄 Liste réinitialisée, on recommence !")
    ...
    embed.set_footer(text=f"GIF {len(used_gifs)}/{len(GIF_LIST)}")
...
async def play(ctx, *, activity: str = None):
    ...
    await ctx.send(f" Je joue maintenant à **{activity}**.")
